...
- **08.09: ширина чату — DEV 3081 експеримент готовий**. Причина неефективного використання екрана знайдена в core UI: `@deepseek-ai/dsh-client-ui-conversation` задає `--dsh-chat-content-width: 748px`, від нього залежать стрічка повідомлень і composer; user bubble окремо обмежений `max-width:min(525px,82%)`. Зроблено окремий локальний client-only comfort-плагін `@local/dsh-wide-chat` у `C:\All\Project\Vibecode\repos\dsh-wide-chat`, підключений тільки до dev: ширина чату `min(1180px, calc(100vw - 520px))`, user bubble `min(860px,88%)`, fallback на upstream для `<=1280px`. Dev restart SUCCESS (PID 27460). Верифікація: `http://127.0.0.1:3081/` HTTP 200, `http://127.0.0.1:3081/plugins/@local/dsh-wide-chat/client.js` HTTP 200, root boot містить `dsh-wide-chat`, client містить `--dsh-chat-content-width`, `1180px`, `1280px`. Чекає живий Ctrl+F5 UX-тест користувача на 3081; після OK — промоція в prod за canary/safe-restart/commit.
