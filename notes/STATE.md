...
- **08.09: українізація сторонніх плагінів на DEV 3081 — перший повний шар готовий**. Через мертвих жор (Kimi weekly limit) робиться вручну/pnpm patch. Готово і верифіковано на dev (restart SUCCESS, bundle містить uk-рядки): `dsh-terminal` (18 ключів), `dock-files` (~44), `dock-editor` (~22 + пошук), `@linxin666/dsh-usage` (мінімальний клієнтський словник ~40), `dock-git` (повний UI/command/menu словник ~147 ключів + detectLocale(uk)), `dsh-classic-coding` (видимі hardcoded UI/status/tooltip рядки; у коді лишились китайські коментарі, не UI). Dev restart після `dock-git`/`classic-coding`: SUCCESS (PID 28828). HTTP-перевірка бандлів: `dock-git` містить `Історія Git` ✓; `dsh-classic-coding` містить `Classic Coding` + `Відкрити редактор файлів` ✓; старих китайських UI-рядків `打开文件编辑器`/`等待会话就绪` у бандлі нема ✓. Серверні pet-бульбашки `@linxin666/dsh-usage` ще окремо. **Промоція в prod виконана 08.09**: коміт 6107cdf, safe-restart SUCCESS (старт 15:58:53, лог завершено 15:59:00, PID 31468); prod 3080 HTTP 200, бандли `dock-git`/`dsh-classic-coding`/`@linxin666/dsh-usage` HTTP 200 і містять українські рядки ✓.
- **08.09: ширина чату — DEV 3081 експеримент v2 готовий**. Перший варіант лише розширив колонку, але screenshot користувача показав головну проблему: assistant-відповідь лишається вузьким prose-блоком з великими порожніми полями, тобто ширина використовується погано. RCA в core UI: `@deepseek-ai/dsh-client-ui-conversation` задає `--dsh-chat-content-width: 748px`, ChatView колонка й composer сидять на цій змінній; user bubble окремо `max-width:min(525px,82%)`; assistant markdown має `font-size:16px; line-height:28px` і gap 16px. Окремий локальний client-only comfort-плагін `@local/dsh-wide-chat` у `C:\All\Project\Vibecode\repos\dsh-wide-chat`, підключений тільки до dev, тепер робить комплексний layout: ширина `min(1440px, calc(100vw - 360px))`, менші вертикальні gaps, assistant row на всю ширину, plain prose обмежено `96ch` для читабельності, code/table/tool/error блоки можуть займати 100%, user bubble `min(760px,74%)`, composer розширюється разом із колонкою, fallback на upstream для `<=1280px`. Dev restart SUCCESS (PID 5424), hardening restart SUCCESS (PID 32332). Верифікація: `http://127.0.0.1:3081/` HTTP 200, boot містить `dsh-wide-chat`; client містить `1440px`, `calc(100vw - 360px)`, `96ch`, `data-chat-flow-kind="assistant-step"`, `data-composer-card`, `min(760px, 74%)`, `columns: 2 34ch`, `max-width: min(100%, 120ch)`, `overflow-x: auto`. Hardening: прибрано over-broad selector `div[class*="root"]`, списки на широких екранах можуть іти у 2 колонки, code/table мають horizontal scroll. Додано UX-перемикач режимів унизу зліва: `balanced → wide → compact`, стан у `localStorage` ключ `dsh-wide-chat.mode`; dev restart SUCCESS (PID 33608), client містить `Chat: balanced`, `Chat: wide`, `Chat: compact`, `data-dsh-wide-chat-mode`, `min(1680px, calc(100vw - 280px))`. Полірування 08.09: перемикач українізовано (`Чат: баланс`, `Чат: широкий`, `Чат: компакт`), додано `aria-live` і aria-label з описом режиму; dev restart SUCCESS (PID 15100), client містить укр. рядки. Чекає живий Ctrl+F5 UX-тест користувача на 3081; після OK — промоція в prod за canary/safe-restart/commit.
...
- 2026-09-08: **операційний баг агента/goal loop: async-кроки можуть виглядати "завислими" після фактичного завершення**. Приклад: commit 6107cdf о 15:58:46, safe-restart старт 15:58:53, лог завершено 15:59:00 (~14 с), але todo лишався `in_progress`, доки користувач не смикнув. Правило: після WMI/фонового старту одразу читати лог/health endpoint, закривати todo в тому ж ході, не залишати пункт "commit/restart" активним без явного job/log watcher.
