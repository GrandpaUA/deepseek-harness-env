...
1. **БАГ "prepare" — ГОЛОВНИЙ, не закритий** (деталі в STATE.md → Нюанси): tool calls
   вбивають хід, текстові ходи працюють, рестарт не лікує, headless з новою сесією
   працює. ПЕРШИЙ КРОК: створи НОВИЙ чат і зроби будь-який tool call. Працює →
   кляті лише старі відновлені сесії (працюємо в нових). Падає → бісект
   плагін-бандлів у DSH-Dev/profiles/headless (додавати бандли з
   profiles/web/package.json по одному до падіння). НЕ закривай без доказу.
