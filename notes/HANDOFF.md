...
1. ~~**БАГ "prepare"**~~ — ЗАКРИТИЙ 2026-09-08 (RCA: dual-URL module identity, друга копія
   `@deepseek-ai/*` у `profiles/web/node_modules`; фікс: `.pnpmfile.cjs` + видалення шару,
   деталі в STATE.md → Нюанси). Бонусом закрито й баг "порожній чат" (history parse) —
   pnpm patch для `dsh-checkpoint-rewind` (старий rc.2-контракт проєкцій).
...
3. **GPT-6 Astra на 3080**: обрати в picker, надіслати тестове повідомлення.
   Якщо помилка — див. свіжий `sessions/*/session.jsonl.zstd` (МУЛЬТИФРЕЙМОВИЙ zstd;
   декодер усіх фреймів — `C:\All\Project\Vibecode\DSH-Dev\dump-session.cjs`).
